...
export default {
  ...
  methods: {
    ...
    async loadRetailerProducts() {
      let blockProducts = []
      try {
        const retailerProducts = await this.$store.state.contractMethods
          .getVeggieBlocksWithStage('Retailer')
          .call();

        console.log('Retailer products: ', retailerProducts)

        for (let i=0; i < retailerProducts.length; i++) {
          let productInfo = await this.$store.state.contractMethods
